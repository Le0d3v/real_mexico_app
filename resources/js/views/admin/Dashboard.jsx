...
export default function Dashboard() {
  ...
  return (
    <>
      <div className="bg-slate-100 min-h-screen">
        ...
        <div className="grid md:grid-cols-4 gap-6 mt-10 mb-10" id="driver_kpis">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm text-gray-500">Tutores Registrados</p>
                <h2 className="text-3xl font-bold text-slate-800 mt-2">
                  {tutores.length}
                </h2>
              </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm text-gray-500">Colegiaturas Vencidas</p>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-md transition">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm text-gray-500">Pendientes del Mes</p>
                <h2 className="text-3xl font-bold text-emerald-600 mt-2">
                  {formatCurrency(totalMetaMes - totalRecaudadoMes)}
                </h2>
              </div>
            </div>
          </div>
        ...
        >
          ...
          <div className="flex justify-between mt-4 text-sm text-slate-600 w-full">
            <span>Recaudado: {formatCurrency(totalRecaudadoMes)}</span>
            <span>Meta mensual: {formatCurrency(totalMetaMes)}</span>
          </div>
        </div>
      ...
      >
        <CreateStudent onClose={() => setEstudianteModal(false)} />
      </Modal>
    </>
  );
}
